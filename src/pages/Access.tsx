  ...
  <div className="bg-background min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {tiers.map((tier, i) => (
            <motion.div
              ...
              className={`p-6 ${
                tier.highlight ? "gold-border gold-glow bg-primary/5" : i === 1 ? "border border-blue-900/30 bg-card" : "border border-muted bg-card"
              }`}
            >
              {tier.highlight && (
              ...
              {tier.features.map((f) => (
                <div key={f.text} className="flex items-center gap-2 mb-3">
                  {f.unlocked ? (
                    ...
                    <MdLock className="text-muted-foreground shrink-0" size={16} />
                  )}
                  <span className={`font-body text-sm ${f.unlocked ? "text-foreground" : "text-muted-foreground"}`}>
                    {f.text}
                  </span>
                </div>
              ))}
    ...
    <section className="py-16 grid-bg">
      <div className="container max-w-3xl">
        ...
        <motion.p {...fadeUp} className="text-center mt-8 font-mono text-[10px] text-muted-foreground tracking-widest">
